...
\begin{abstract}
%\boldmath
In \DIFdelbegin \DIFdel{wireless local area networks (WLANs), a media access protocol arbitrates the access to the channel.
In current IEEE 802.11 WLANs, carrier sense multiple access with collision avoidance (}\DIFdelend \DIFaddbegin \DIFadd{this paper we have summarized a family of MAC protocols that offer significant performance improvement over }\DIFaddend CSMA/CA\DIFdelbegin \DIFdel{) is used. Carrier sense multiple access with enhanced collision avoidance (}\DIFdelend \DIFaddbegin \DIFadd{. In its most basic form, }\DIFaddend CSMA/ECA \DIFdelbegin \DIFdel{) is a subtle variant of the well-known }\DIFdelend \DIFaddbegin \DIFadd{achieves this performance boost by simply using a deterministic backoff after each successful transmission and reverting back to the }\DIFaddend CSMA/CA \DIFdelbegin \DIFdel{algorithm that offers substantial performance benefits.
CSMA/ECA significantly reduces the collision probability and, under }\DIFdelend \DIFaddbegin \DIFadd{random behavior when a collision is detected. Under }\DIFaddend certain conditions, \DIFdelbegin \DIFdel{leads to a completely }\DIFdelend \DIFaddbegin \DIFadd{this leads to the construction of a }\DIFaddend collision-free \DIFdelbegin \DIFdel{schedule.
The only difference between CSMA/CA and }\DIFdelend \DIFaddbegin \DIFadd{deterministic schedule in a completely distributed fashion. We have then discussed possible variations to }\DIFaddend CSMA/ECA \DIFdelbegin \DIFdel{is that the latter uses a deterministic backoff after successful transmissions.
This deterministic backoff is a constant and is the same for all the stations. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The first part of the paper is of tutorial nature, offering an introduction to the basic operation of }\DIFdelend \DIFaddbegin \DIFadd{that can further improve the performance under more realistic conditions. }\DIFaddend CSMA/ECA and \DIFdelbegin \DIFdel{describing the benefits of this approach in a qualitative manner.
The second part of the paper surveys related contributions, briefly summarizing the main challenges and potential solutions, and also introducing variants and derivatives of CSMA}\DIFdelend \DIFaddbegin \DIFadd{its variants represent a simple evolution of the currently prevalent protocol CSMA}\DIFaddend /\DIFdelbegin \DIFdel{ECA}\DIFdelend \DIFaddbegin \DIFadd{CA, thus offering backward compatibility and fair coexistence with already deployed hardware. 
}

\DIFadd{Similar techniques can be used to address other problems in wireless networking, such as channel assignment, spreading
code assignment, and channel sensing-order assignment in cognitive radio}\DIFaddend .
\end{abstract}
...
%DIF < In the Aloha protocol, a station transmits as soon as it receives a packet from the upper layers of the protocol stack.
%DIF < A caveat of random media access protocols is that it is possible that at some point of time there is more than one station accessing the channel.
%DIF < If this is the case, the resultant interference may prevent the correct reception of the transmitted packet.
%DIF < A collision is simply a packet loss that is caused by totally or partially overlapping transmissions.
%DIF < If the transmission is not successful, either due to a collision or because of unfavorable channel conditions, the transmitting station will retransmit the same packet after a random backoff time.
...
%DIF < In this paper, we will cover random (and semi-random) multiple access protocols that are faithful to the simplicity, low overhead and distributed nature of the early Aloha protocol.
...
In doing so, it decreases the chances of collisions and, under the assumption of fixed packet length that perfectly fits into the aforementioned fixed slot length, it doubles the throughput of the original Aloha.

Reservation Aloha \cite{crowther1973sbc,tasaka1983spr} extends Slotted Aloha with a reservation mechanism.
Reservation Aloha was intended for random access satellite communications and assumes that the stations listen continuously to the channel and can distinguish between occupied and free slots.
In  Reservation Aloha, a number of consecutive  time slots are grouped in a frame.
The frames are useful in the reservation process and they all contain the same number of slots.
When a station successfully transmits in a given slot of a frame, it implicitly makes a reservation on the same slot of the following frame.
This reservation can be very advantageous in some networks, increasing the capacity and reducing the delay.
In Reservation Aloha, different nodes implicitly agree on a collision-free schedule that results in a better utilization of the shared channel.
...
Both Slotted Aloha and Reservation Aloha use fixed-sized slots.
...
%DIF < We will also constrain our discussion to those scenarios in which CSMA is applicable: WLANs in which the devices are within one another's transmission range.
...
Then we will review different related contributions to discuss  the research challenges, the performance gains, and the scenarios of interest and applicability.
We will also \DIFdelbegin \DIFdel{shortly }\DIFdelend \DIFaddbegin \DIFadd{briefly }\DIFaddend mention the generality of the underlying principle and the possibility of using it for diverse problems in the field of radio resource management.
...
%DIFDELCMD < %%%
\DIFdelend The remainder of the paper is organized as follows.
...
To reduce the likelihood of collisions\DIFaddbegin \DIFadd{, }\DIFaddend in CSMA/CA, the channel is divided into slots and transmissions are synchronized to slot borders and preceded by a random backoff.
...
\DIFdelbegin \DIFdel{Then, the stations decrement the backoff values by one in every slot.
}\DIFdelend The transmission occurs when the backoff counter reaches zero.
...
%DIF < This is the key idea in CSMA/ECA and it is illustrated by the following example.
...
The \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend of CSMA/CA and CSMA/ECA for a network of two nodes is depicted in Fig.~\ref{fig:ca_vs_eca}.
...
It is useful to compare the \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend of CSMA/CA in Fig.~\ref{fig:csma_ca} to the \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend of CSMA/ECA in Fig.~\ref{fig:csma_eca}.
The initial \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend is exactly the same for the two protocols: a collision occurs and a random backoff is selected.
...
From this point on, the \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend of the system is collision-free, deterministic, cyclic and fair.
The cycle length is indicated in the figure, and it is easy to observe that the \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend of the system in the second cycle is exactly the same as in the first cycle.
...
%DIF < It is notable that the collision reduction and the performance improvement is obtained with no extra cost in terms of complexity, signaling or overhead.
...
%DIFDELCMD < %%%
\DIFdelend Even though there are clear similarities between CSMA/ECA and Reservation Aloha, there are also two remarkable differences.
...
\label{sec:survey}
%DIF < It is not the intention of this section to provide an exhaustive survey of all the contributions that deal with the construction of a collision-free schedule.
...
%DIF < Some of the issues we will be looking at are the underlying mathematical framework, backward compatibility, operation in the presence of errors, operation in networks with a very large number of contenders and the performance advantages of using a deterministic backoff after successful transmissions. 
...
%DIF < We will take a closer look into these two contributions.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Notice that, if the protocol is used as described in the previous section, there is a maximum number of stations that can be accommodated in a collision-free fashion.
...
%DIF < If there are more pigeons than pigeonholes, then there is at least one pigeonhole with two pigeons.
\DIFdelend 

%DIF < Obviously, a mechanism that dynamically adjusted the number of columns (i.e., the value of the deterministic backoff after successes) as a function of the number of contending stations would solve the issue.
...
If a deterministic station \DIFdelbegin \DIFdel{backoff }\DIFdelend collides with a random station, only one of them needs to choose a random backoff to prevent a new collision of the two stations in their next transmission attempt.
In fact, switching the deterministic station to random \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend only increases the chance of collision with other deterministic stations.
...
Firstly, it converges faster to collision-free operation, as a station that successfully transmits once never switches back to the random \DIFdelbegin \DIFdel{behaviour}\DIFdelend \DIFaddbegin \DIFadd{behavior}\DIFaddend .
Secondly, once a collision-free schedule is built, it is not possible for a channel error to move the system back to the random \DIFdelbegin \DIFdel{behaviour}\DIFdelend \DIFaddbegin \DIFadd{behavior}\DIFaddend .
And thirdly, after a collision-free schedule has been built, it is not possible \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend a new entrant \DIFdelbegin \DIFdel{destroys }\DIFdelend \DIFaddbegin \DIFadd{to destroy }\DIFaddend the schedule.
The new entrant will simply use a random \DIFdelbegin \DIFdel{behaviour }\DIFdelend \DIFaddbegin \DIFadd{behavior }\DIFaddend (possibly suffering collisions) until it successfully transmits.

The problem is that real clocks may suffer drifts, that can result in slot \DIFdelbegin \DIFdel{drift }\DIFdelend \DIFaddbegin \DIFadd{misalignment }\DIFaddend \cite{gong2012asd}.
...
This paper presents an analytical model and simulations that use realistic channel realizations and \DIFdelbegin \DIFdel{the }\DIFdelend Automated Rate Fallback (ARF).
...
%DIF < The paper compares CSMA/ECA and CSMA/CA with and without the Request-To-Send/Clear-To-Send (RTS/CTS) four-way handshake.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{ARF is introduced }\DIFdelend \DIFaddbegin \DIFadd{ARF is a mechanism used }\DIFaddend to adapt the transmission rate to the channel conditions and \DIFdelbegin \DIFdel{it reduces }\DIFdelend \DIFaddbegin \DIFadd{simply works by reducing }\DIFaddend the transmission rate after unsuccessful transmissions.
This approach does not work well with CSMA/CA when collisions occur, as ARF misinterprets \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{all }\DIFaddend failures as channel errors and reduces the transmission rate\DIFaddbegin \DIFadd{, which further worsens the performance when failures are due to collisions}\DIFaddend .
This problem can be alleviated by using RTS/CTS that can differentiate between collisions and channel errors\DIFdelbegin \DIFdel{, but }\DIFdelend \DIFaddbegin \DIFadd{.
However }\DIFaddend RTS/CTS penalizes the performance due to the \DIFdelbegin \DIFdel{overheads additional }\DIFdelend \DIFaddbegin \DIFadd{additional overheads}\DIFaddend .
CSMA/ECA solves the problem by preventing \DIFdelbegin \DIFdel{collision}\DIFdelend \DIFaddbegin \DIFadd{collisions}\DIFaddend , without adding any additional overhead.
...
%DIFDELCMD < %%%
\DIFdelend \section{Conclusion} \label{sec:conclusion}

In this paper we have discussed the use of a deterministic backoff after \DIFdelbegin \DIFdel{collisions }\DIFdelend \DIFaddbegin \DIFadd{successful transmission }\DIFaddend to prevent collisions in WLANs and \DIFaddbegin \DIFadd{to }\DIFaddend deliver substantial performance benefits.
The deterministic backoff represents a simple evolution to the currently used protocol, thus offering backward compatibility and fair coexistence with already deployed hardware.


Similar techniques have been used to address other problems in wireless networking such as channel assignment, spreading code assignment and channel sensing order assignment in cognitive radio.
\DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF < By doing so, it is guaranteed that stations that have successfully transmitted in their last transmission attempt will not collide among them in their next transmission attempt.
...
%DIFDELCMD < %%%
%DIF < In the second part of the paper we have reviewed different contributions that either assess the performance of the system in non-ideal conditions or suggest improvements to deal with the non-idealities.
...
%DIFDELCMD < %%%
%DIF < As a summary, we have provided an overview of a promising approach that could substantially reduce the problem of collisions in next generation WLANs.
\DIFdelend 
