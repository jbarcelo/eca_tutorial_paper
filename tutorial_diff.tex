...
}\DIFaddend Consequently, the protocol might be improved if deterministic stations kept using a deterministic backoff \DIFaddbegin \DIFadd{even }\DIFaddend after collisions.
...
%DIFDELCMD < %%%
\DIFdel{An exponential packet length distribution is considered and the curves that compare }\DIFdelend \DIFaddbegin \DIFadd{.
For comparison, results are also presented for }\DIFaddend CSMA/CA \DIFdelbegin \DIFdel{and CSMA}\DIFdelend \DIFaddbegin \DIFadd{with and without the Request-To-Send}\DIFaddend /\DIFdelbegin \DIFdel{E2CA (See Fig.~\ref{fig:performance}) show a substantial throughput improvement, for both the BA and }\DIFdelend \DIFaddbegin \DIFadd{Clear-To-Send (}\DIFaddend RTS/CTS\DIFdelbegin \DIFdel{the scheme.
...
\DIFdelbegin \DIFdel{The purpose of ARF is }\DIFdelend %DIF > The paper compares CSMA/ECA and CSMA/CA with and without the Request-To-Send/Clear-To-Send (RTS/CTS) four-way handshake.
\DIFaddbegin 

\DIFadd{ARF is introduced }\DIFaddend to adapt the transmission rate to the channel conditions \DIFdelbegin \DIFdel{.
In unfavorable channel conditions, ARF uses slower (and more robust) transmission rates.
The ARF mechanism simply switches to slower transmission rates }\DIFdelend \DIFaddbegin \DIFadd{and it reduces the transmission rate }\DIFaddend after unsuccessful transmissions\DIFdelbegin \DIFdel{, as it believes that they are an indication of unfavorable channel conditions.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The problem is that, when BA is used, ARF cannot differentiate between }\DIFdelend \DIFaddbegin \DIFadd{.
This approach does not work well with CSMA/CA when collisions occur, as ARF misinterprets the failures as }\DIFaddend channel errors and \DIFdelbegin \DIFdel{collisions.
In any case, it slows down }\DIFdelend \DIFaddbegin \DIFadd{reduces }\DIFaddend the transmission rate.
\DIFdelbegin \DIFdel{When ARF is combined with CSMA}\DIFdelend \DIFaddbegin \DIFadd{This problem can be alleviated by using RTS}\DIFaddend /\DIFdelbegin \DIFdel{CA in scenarios with a large number of contenders, the high number of collisions slows down the transmission rate and therefore the performance is doubly penalized: firstly because of the collisions and secondly because of the slower rates.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This problem is apparent in the BA curve }\DIFdelend \DIFaddbegin \DIFadd{CTS that can differentiate between collisions and channel errors, but RTS/CTS penalizes the performance due to the overheads additional.
CSMA/ECA solves the problem by preventing collision, without adding any additional overhead.
The curves }\DIFaddend in Fig.~\ref{fig:performance} \DIFdelbegin \DIFdel{.
...
Therefore }\DIFdelend \DIFaddbegin \DIFadd{(reproduced from }\cite{martorell2012pec}\DIFadd{) show that }\DIFaddend CSMA/\DIFdelbegin \DIFdel{CA-RTS/CTS }\DIFdelend \DIFaddbegin \DIFadd{ECA }\DIFaddend outperforms CSMA/\DIFdelbegin \DIFdel{CA-BA when the number of contenders is large.
The best performance is obtained by }\DIFdelend \DIFaddbegin \DIFadd{CA and also }\DIFaddend CSMA/\DIFdelbegin \DIFdel{E2CA-BA as it prevents collisions and does not suffer the overhead of the extra }\DIFdelend \DIFaddbegin \DIFadd{CA with }\DIFaddend RTS/CTS\DIFdelbegin \DIFdel{packets}\DIFdelend .
