...
Carrier sense multiple access with enhanced collision avoidance (CSMA/ECA) is a subtle variant of the well-known CSMA/CA algorithm that offers substantial performance benefits.
\DIFdelbegin \DIFdel{In }\DIFdelend CSMA/ECA \DIFdelbegin \DIFdel{, the number of collisions is initially reduced before the different stations construct a }\DIFdelend \DIFaddbegin \DIFadd{significantly reduces the collision probability and, under certain conditions, leads to a completely }\DIFaddend collision-free schedule.
...
The second part of the paper surveys \DIFdelbegin \DIFdel{the main contributionsto this research field}\DIFdelend \DIFaddbegin \DIFadd{related contributions}\DIFaddend , briefly summarizing the main challenges and potential solutions, and also introducing variants and derivatives of CSMA/ECA.
\DIFdelbegin \DIFdel{Finally, simulation results confirm the advantages of the proposed approach.
...
\IEEEPARstart{T}{he} distributed sharing of a medium by multiple stations is a classic communications problem. 
...
In doing so, it decreases the chances of collisions and, under the assumption of fixed packet \DIFdelbegin \DIFdel{size}\DIFdelend \DIFaddbegin \DIFadd{length that perfectly fits into the above mentioned fixed slot length}\DIFaddend , it doubles the throughput of the original Aloha.
\DIFaddbegin 

\DIFaddend Reservation Aloha \cite{crowther1973sbc,tasaka1983spr} extends Slotted Aloha with a reservation mechanism.
\DIFaddbegin \DIFadd{Reservation Aloha was intended for random access satellite communications and assumes that the stations listen continuously to the channel and can identify which slots are occupied and which slots are free.
}\DIFaddend In  Reservation Aloha, a number of consecutive  time slots is grouped in a frame.
\DIFdelbegin \DIFdel{All frames }\DIFdelend \DIFaddbegin \DIFadd{The frames are useful in the reservation process and they all }\DIFaddend contain the same number of slots\DIFdelbegin \DIFdel{and can be used for reservation purposes}\DIFdelend .
...
\DIFdelbegin \DIFdel{WLANs }\DIFdelend \DIFaddbegin \DIFadd{Wireless Local Area Networks (WLANs) }\DIFaddend represent a different scenario because the distances are much shorter.
...
It is possible to shorten the empty slots in this case because\DIFaddbegin \DIFadd{, }\DIFaddend when stations are close to each other\DIFaddbegin \DIFadd{, }\DIFaddend they can quickly detect whether a slot is busy or empty by simply sensing the medium at the beginning of each slot.
This technique is called \DIFdelbegin \DIFdel{CSMA and  it }\DIFdelend \DIFaddbegin \DIFadd{Carrier Sense Multiple Access (CSMA) and }\DIFaddend is used in WLANs that implement the IEEE 802.11 standard.
...
\DIFdelbegin \DIFdel{We will also constrain our discussion to those scenarios in which CSMA is applicable: WLANs in which the devices are within one another's transmission range.
}\DIFdelend %DIF > We will also constrain our discussion to those scenarios in which CSMA is applicable: WLANs in which the devices are within one another's transmission range.

Recently, different research initiatives have suggested to combine the advantages of Reservation Aloha \DIFdelbegin \DIFdel{networks and CSMA networks }\DIFdelend \DIFaddbegin \DIFadd{and CSMA }\DIFaddend (e.g., \cite{barcelo2008lba,he2009srb,barcelo2010fcc,fang2011dlm,barcelo2011tcf,martorell2012pec}). 
...
We will describe CSMA/ECA \DIFaddbegin \DIFadd{(where ECA stands for Enhanced Collision Avoidance) }\DIFaddend as an example of a contention protocol that uses a deterministic backoff after successful transmissions to reduce the number of collisions.
Then we will review different \DIFdelbegin \DIFdel{contributions in this research field }\DIFdelend \DIFaddbegin \DIFadd{related contributions }\DIFaddend to discuss  which are the research challenges, the performance gains\DIFaddbegin \DIFadd{, }\DIFaddend and the scenarios of interest and applicability.
...
The paper \DIFdelbegin \DIFdel{finishes }\DIFdelend \DIFaddbegin \DIFadd{ends }\DIFaddend with some concluding remarks in Section~\ref{sec:conclusion}.
...
%DIF < 
%DIF < A careful observation of the CSMA/CA example in the figure reveals that the stations do not make any effort to construct a collision free schedule.
%DIF < On the contrary, each station behaves randomly, making it impossible for the other station to make any prediction or gain any knowledge to reduce the chances of collision.
%DIF < As a result, CSMA/CA suffers from a high collision rate that undermines the overall network performance.
%DIF < The presence of collision slots is much more harmful than the presence of empty slots, since collision slots are much longer.
%DIF < In contrast to wired networks such as Ethernet, in WLANs it is not possible to detect the collision until the transmission is completed.
%DIF < Therefore, the colliding stations keep on transmitting the whole packet and the duration of the collision is equal to the duration of the longest of the transmissions involved in the collision.
...
%DIF < In fact, by choosing the same deterministic backoff, these two stations are in order to prevent a collision in the future.
...
The general rule is that \DIFdelbegin \DIFdel{collision free }\DIFdelend \DIFaddbegin \DIFadd{collision-free }\DIFaddend operation is reached after all the contending stations successfully transmit \DIFdelbegin \DIFdel{in the same cycle }\DIFdelend \DIFaddbegin \DIFadd{within a single cycle duration}\DIFaddend .
...
For convenience, we draw all the slots with equal length\DIFdelbegin \DIFdel{, even though in reality their length can differ by orders of magnitude}\DIFdelend .
...
As \DIFdelbegin \DIFdel{it is }\DIFdelend pointed out in \cite{fang2011dlm}, \DIFdelbegin \DIFdel{it is better to have }\DIFdelend \DIFaddbegin \DIFadd{having }\DIFaddend a schedule that is larger than the number of contenders \DIFaddbegin \DIFadd{is better }\DIFaddend than having one that is shorter.
The reason is that empty slots are much shorter than collision slots \DIFaddbegin \DIFadd{and therefore, idle waiting is far less costly than collisions}\DIFaddend .
...
%Another advantage of CSMA/ECA is its close similarity to CSMA/CA the protocol that is used in most of the currently deployed devices.
%This similarity facilitates for the widespread implementation of CSMA/ECA in next-generation devices.
...
}\DIFdelend %DIF > Some of the issues we will be looking at are the underlying mathematical framework, backward compatibility, operation in the presence of errors, operation in networks with a very large number of contenders and the performance advantages of using a deterministic backoff after successful transmissions. 
...
}\DIFdelend %DIF > Notice that, if the protocol is used as described in the previous section, there is a maximum number of stations that can be accommodated in a collision-free fashion.
%DIF > In other words, if there were more stations than columns in Fig.~\ref{fig:csma_eca_compact}, it would not be possible to assign the stations to columns in such a way that there would be a single station in each column.
%DIF > This is the counting argument that is often referred to as the pigeonhole principle.
...
%DIF < A different approach to stickiness is presented in \cite{barcelo2011tcf}, where  CSMA/E2CA is introduced. 
%DIF < CSMA/E2CA is a variant of CSMA/ECA in which a deterministic backoff is used for two consecutive times after each successful packet transmission.
%DIF < Note that while the approach to stickiness in \cite{fang2011dlm} is probabilistic, in CSMA/E2CA it is deterministic.
%DIF < This deterministic nature can be appealing for manufacturers, since it simplifies the implementation.
%DIF < CSMA/E2CA also provides all the advantages that come with stickiness: shorter convergence time and further schedule resistance to channel errors and new entrants.
\DIFaddbegin \subsection{\DIFadd{Stickiness for faster convergence and increased robustness}}
\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{More stickiness for faster convergence and increased robustness}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{If CSMA/ECA is used as described in Sect.~\ref{sec:eca}, a single channel error or the entry of new contender can disrupt collision-free operation.
To address this problem, stickiness was introduced in }%DIFDELCMD < \cite{fang2011dlm} %%%
\DIFdel{and later discussed in }%DIFDELCMD < \cite{barcelo2011tcf} %%%
\DIFdel{in a protocol called CSMA/E2CA.
In CSMA/E2CA}\DIFdelend \DIFaddbegin \DIFadd{Notice that, in ideal conditions}\DIFaddend , a station \DIFdelbegin \DIFdel{that is }\DIFdelend using a deterministic backoff \DIFdelbegin \DIFdel{will only switch to a random behaviour if it suffers two unsuccessful transmissions in two consecutive transmission attempts.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The operation of CSMA/E2CA is illustrated in Fig.~\ref{fig:csma_e2ca}.
In this figure, there is }\DIFdelend \DIFaddbegin \DIFadd{may collide only with a station using a random backoff.
If }\DIFaddend a station that \DIFdelbegin \DIFdel{successfully transmits in slot~11.
This station uses }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend a deterministic backoff \DIFdelbegin \DIFdel{and then it suffers an unsuccessful  transmission in its next transmission attempt in slot 27.
Despite the collision, this station sticks to using a deterministic backoff because the CSMA/E2CA protocol states that a deterministic backoffis used for two consecutive times after a successful transmission.
Compare this behaviour to the behaviour observed in Fig.
~\ref{fig:csma_eca_compact} where the station that successfully transmits in slot 11, suffers a collision in slot 27 and then causes another collision after using a }\DIFdelend \DIFaddbegin \DIFadd{collides with a station that used a }\DIFaddend random backoff\DIFdelbegin \DIFdel{and transmitting in slot 45.
}%DIFDELCMD < 
...
\DIFdelend \DIFaddbegin \DIFadd{, only one of them needs to choose a random backoff to prevent a new collision of the two stations in their next transmission attempt.
...
The property of using a deterministic backoff after suffering collisions is called in ``stickyness'' }\cite{fang2011dlm}\DIFadd{.
}\DIFaddend 

\DIFdelbegin \DIFdel{The extra stickiness }%DIFDELCMD < \cite{barcelo2011tcf} %%%
\DIFdel{offers three advantages.
...
Second, it prevents cascading or chain-reaction effects in which the entry of single user completely breaks }\DIFdelend \DIFaddbegin \DIFadd{In ideal conditions, this solution has several advantages.
...
Secondly, once }\DIFaddend a collision-free schedule \DIFdelbegin \DIFdel{.
And third, it makes the schedule robust to channel errors.
}\DIFdelend \DIFaddbegin \DIFadd{is built, it is not possible that a channel error moves the system back to the random behaviour.
...
\DIFdelbegin \DIFdel{Under the assumption of ideal clocks, increasing the stickiness also increases the robustness.
...
}\DIFdelend The problem is that real clocks \DIFdelbegin \DIFdel{suffer clock drifts}\DIFdelend \DIFaddbegin \DIFadd{may suffer drifts, that can result in slot drift }\DIFaddend \cite{gong2012asd}\DIFdelbegin \DIFdel{and, as a result, two stations that have successfully transmitted in consecutive slots may end up colliding in the next transmission slot.
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Performance of CSMA/ECA and CSMA/E2CA}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A detailed study of the use of }\DIFdelend \DIFaddbegin \DIFadd{.
Two stations using a deterministic backoff may collide if one of them miscounts the number of slots.
This is a very undesirable situation if both stations ``stick'' to the use of }\DIFaddend a deterministic backoff after \DIFdelbegin \DIFdel{successful transmissions is offered in }%DIFDELCMD < \cite{he2009srb}%%%
\DIFdel{. 
One of the characteristics of }\DIFdelend \DIFaddbegin \DIFadd{suffering a collision.
...
\DIFadd{To attain the advantages of stickiness while preventing the potential pitfall, }\cite{fang2011dlm} \DIFadd{proposed probabilistic stickiness and }\cite{barcelo2011tcf} \DIFadd{proposed finite stickiness, in which deterministic stations switch back to random behaviour after a given number of collisions.
}\DIFaddend CSMA/\DIFdelbegin \DIFdel{ECA is that reaching a collision-free schedule may require several transmission rounds.
The network goes through a transient state in which some of the stations use }\DIFdelend \DIFaddbegin \DIFadd{E2CA in }\cite{barcelo2011tcf} \DIFadd{moves back to random behaviour after two consecutive collisions.
}

\DIFadd{Fig.~\ref{fig:csma_e2ca} illustrates the operation of the protocol when deterministic stations ``stick'' to }\DIFaddend a deterministic backoff \DIFdelbegin \DIFdel{while the remaining stations use a random backoff.
When the }\DIFdelend \DIFaddbegin \DIFadd{after suffering a collision.
It can be observed that in this example, the convergence to }\DIFaddend collision-free \DIFdelbegin \DIFdel{schedule is eventually reached, the system moves to a deterministic steady state.}\DIFdelend \DIFaddbegin \DIFadd{operation is faster than in Fig.~\ref{fig:csma_eca_compact}.
}

\subsection{\DIFadd{Performance of CSMA/ECA and CSMA/E2CA}}

\DIFaddend An analytical model of the \DIFdelbegin \DIFdel{system is presented }\DIFdelend \DIFaddbegin \DIFadd{expected number of slots required to reach collision-free operation is introduced in }\DIFaddend \cite{he2009srb}\DIFdelbegin \DIFdel{which can be used to make predictions about the length of the transient state.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend The paper also presents a comprehensive simulation study which includes realistic ingredients such as traffic differentiation, carrier sense errors, and channel errors.
...
Interestingly, the authors \DIFdelbegin \DIFdel{remark }\DIFdelend \DIFaddbegin \DIFadd{report }\DIFaddend that the implementation of the proposed, protocol in the well-known simulator NS-2 required the change of only three lines of code.
...
This paper presents an analytical model and simulations that use realistic channel realizations and the automated rate fallback \DIFaddbegin \DIFadd{(ARF) }\DIFaddend mechanism for the selection of the modulation \DIFaddbegin \DIFadd{and channel }\DIFaddend coding scheme.
\DIFaddbegin \DIFadd{The paper offers results for the two handshaking mechanisms in IEEE 802.11: Basic Access (BA) and request-to-send/clear-to-send (RTS/CTS).
%DIF > BA is simpler as it includes only a transmission and an acknowledgement.
%DIF > In RTS/CTS, the transmission and the acknowledgement are preceded by the RTS and CTS signaling frames.
}

\DIFaddend An exponential packet length distribution is considered and the curves that compare CSMA/CA and CSMA/E2CA (See Fig.~\ref{fig:performance}) show a substantial throughput improvement, for both the \DIFdelbegin \DIFdel{basic access (BA ) and the request-to-send}\DIFdelend \DIFaddbegin \DIFadd{BA and RTS}\DIFaddend /\DIFdelbegin \DIFdel{clear-to-send (}\DIFdelend \DIFaddbegin \DIFadd{CTS the scheme.
The details of the study are available in }\cite{martorell2012pec}\DIFadd{.
}

\DIFadd{The purpose of ARF is to adapt the transmission rate to the channel conditions.
...
\DIFadd{The problem is that, when BA is used, ARF cannot differentiate between channel errors and collisions.
...
\DIFadd{This problem is apparent in the BA curve in Fig.~\ref{fig:performance}.
The use of }\DIFaddend RTS/CTS \DIFdelbegin \DIFdel{) scheme}\DIFdelend \DIFaddbegin \DIFadd{involves additional handshaking control frames and makes it possible to differentiate between channel errors and collisions.
Therefore CSMA/CA-RTS/CTS outperforms CSMA/CA-BA when the number of contenders is large.
The best performance is obtained by CSMA/E2CA-BA as it prevents collisions and does not suffer the overhead of the extra RTS/CTS packets}\DIFaddend .
...
All \DIFdelbegin \DIFdel{that }\DIFdelend it needs is for the stations to use a deterministic backoff after successful transmissions.

\DIFdelbegin \DIFdel{By doing so, it is guaranteed that two stations that have successfully transmitted in their last transmission attempt will not collide with each other their next transmission attempt.
...
}\DIFdelend %DIF > By doing so, it is guaranteed that stations that have successfully transmitted in their last transmission attempt will not collide among them in their next transmission attempt.
%DIF > Furthermore, in ideal conditions, the system will reach a steady-state in which all the stations transmit in a round-robin fashion in a collision-free schedule.
...
}\DIFdelend %DIF > As a summary, we have provided an overview of a promising approach that could substantially reduce the problem of collisions in next generation WLANs.
