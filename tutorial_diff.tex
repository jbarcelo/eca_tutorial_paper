...
\DIFaddbegin \DIFadd{The original Aloha network was designed for inter-island communication.
...
}\DIFaddend In those networks where the propagation times are short compared to the duration of the transmission of a packet and all the stations can hear each other's transmissions, empty slots can be much shorter than busy slots.
...
\DIFaddbegin \DIFadd{As pointed }\DIFaddend Some of the provided references point out that the general framework that \DIFdelbegin \DIFdel{that }\DIFdelend we use in our description of CSMA/ECA has applicability to other problems in the networking and communications field, such as network coding and channel assignment.
...
Most of \DIFaddbegin \DIFadd{the }\DIFaddend currently deployed WLANs are compliant with the IEEE 802.11 standard and rely on CSMA with collision avoidance (CSMA/CA) to share the channel time.
...
Another advantage of CSMA/ECA is \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend close similarity to \DIFaddbegin \DIFadd{CSMA/CA }\DIFaddend the protocol that is used in \DIFaddbegin \DIFadd{most of the }\DIFaddend currently deployed devices\DIFdelbegin \DIFdel{, CSMA/CA}\DIFdelend .
This similarity \DIFdelbegin \DIFdel{should pave the way }\DIFdelend \DIFaddbegin \DIFadd{facilitates }\DIFaddend for the widespread implementation of CSMA/ECA in next-generation devices.
...
In this section we will \DIFdelbegin \DIFdel{comment on }\DIFdelend \DIFaddbegin \DIFadd{summarize }\DIFaddend a small subset of contributions to offer an overview of some of the problems and \DIFaddbegin \DIFadd{possible }\DIFaddend enhancements of the basic idea that we have described in the previous section. 
...
In other words, a station }\DIFdelend that is using a deterministic backoff will only switch to a random behaviour if it suffers two \DIFdelbegin \DIFdel{contiguous }\DIFdelend \DIFaddbegin \DIFadd{consecutive }\DIFaddend unsuccessful transmissions.
...
\DIFaddbegin \DIFadd{With the use of stickiness, the constructed schedule is much more robust, as deterministic stations will not move back to random behaviour after a single unsuccessful transmission.
