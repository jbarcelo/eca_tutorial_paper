...
CSMA/ECA significantly reduces the collision probability and, under certain conditions, leads to a completely collision-free schedule.
...
The second part of the paper surveys related contributions, briefly summarizing the main challenges and potential solutions, and also introducing variants and derivatives of CSMA/ECA.
\end{abstract}
...
In doing so, it decreases the chances of collisions and, under the assumption of fixed packet length that perfectly fits into the \DIFdelbegin \DIFdel{above mentioned }\DIFdelend \DIFaddbegin \DIFadd{aforementioned }\DIFaddend fixed slot length, it doubles the throughput of the original Aloha.

Reservation Aloha \cite{crowther1973sbc,tasaka1983spr} extends Slotted Aloha with a reservation mechanism.
Reservation Aloha was intended for random access satellite communications and assumes that the stations listen continuously to the channel and can \DIFdelbegin \DIFdel{identify which slots are occupied and which slots are free }\DIFdelend \DIFaddbegin \DIFadd{distinguish between occupied and free slots}\DIFaddend .
In  Reservation Aloha, a number of consecutive  time slots \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend grouped in a frame.
The frames are useful in the reservation process and they all contain the same number of slots.
\DIFdelbegin \DIFdel{As an example, when }\DIFdelend \DIFaddbegin \DIFadd{When }\DIFaddend a station successfully transmits in \DIFdelbegin \DIFdel{the first }\DIFdelend \DIFaddbegin \DIFadd{a given }\DIFaddend slot of a frame, it implicitly makes a reservation on the \DIFdelbegin \DIFdel{first }\DIFdelend \DIFaddbegin \DIFadd{same }\DIFaddend slot of the following frame.
This reservation can be very advantageous in some networks, increasing the capacity and reducing the delay.
In Reservation Aloha, \DIFdelbegin \DIFdel{the }\DIFdelend different nodes implicitly agree on a collision-free schedule that results in a better utilization of the \DIFdelbegin \DIFdel{common }\DIFdelend \DIFaddbegin \DIFadd{shared }\DIFaddend channel.
...
Both Slotted Aloha and Reservation Aloha use \DIFdelbegin \DIFdel{fixed size }\DIFdelend \DIFaddbegin \DIFadd{fixed-sized }\DIFaddend slots.
...
This way the network performance can be improved, since the channel will be idle for a smaller fraction of time and therefore\DIFaddbegin \DIFadd{, }\DIFaddend there will be more time for successful transmissions.
...
The novelty of our proposed protocol is that long collision slots are avoided\DIFaddbegin \DIFadd{, }\DIFaddend thus substantially increasing the network throughput.
...
Then we will review different related contributions to discuss  \DIFdelbegin \DIFdel{which are }\DIFdelend the research challenges, the performance gains, and the scenarios of interest and applicability.
...
Since the stations can use carrier sensing to detect the end of a transmission, it is possible to synchronize the nodes to the end of \DIFdelbegin \DIFdel{that variable length transmission}\DIFdelend \DIFaddbegin \DIFadd{variable length transmissions}\DIFaddend .
...
For this reason, the length of a collision slot is approximately equal to the length of the longest \DIFdelbegin \DIFdel{of the different packet transmissions }\DIFdelend \DIFaddbegin \DIFadd{transmission }\DIFaddend involved in the collision.

%DIF < The collision avoidance mechanism of CSMA/CA requires the stations to precede transmissions by a random backoff.
...
In particular, \DIFdelbegin \DIFdel{the stations randomly choose a backoff value and }\DIFdelend \DIFaddbegin \DIFadd{stations performing backoff }\DIFaddend set a backoff counter to \DIFdelbegin \DIFdel{that value }\DIFdelend \DIFaddbegin \DIFadd{a randomly chosen value and decrement it by one at every slot}\DIFaddend .
...
There are six different stations competing for the channel and the hatching pattern of each disk identifies \DIFdelbegin \DIFdel{which is }\DIFdelend the transmitting station.
...
%DIF < In CSMA/CA all the transmissions are preceded by a random backoff, and therefore no collision-free schedule is reached no matter how long the system runs.
%DIF < On the other hand, in CSMA/ECA the stations use a deterministic backoff after successful transmissions.
...
Even though CSMA/ECA was initially suggested to prevent collisions in WLANs, the underlying mathematical framework is applicable to \DIFdelbegin \DIFdel{a variety of }\DIFdelend \DIFaddbegin \DIFadd{various }\DIFaddend resource allocation problems in the field of wireless networking, such as cognitive radio \cite{khan2013aso}, channel selection and network coding \cite{duffy2011dcs}.
...
\DIFdelbegin \DIFdel{Notice that, in }\DIFdelend \DIFaddbegin \DIFadd{Under }\DIFaddend ideal conditions, a station using a deterministic backoff may collide only with a station using a random backoff.
If a \DIFdelbegin \DIFdel{station that used a deterministic }\DIFdelend \DIFaddbegin \DIFadd{deterministic station }\DIFaddend backoff collides with a \DIFdelbegin \DIFdel{station that used a random backoff}\DIFdelend \DIFaddbegin \DIFadd{random station}\DIFaddend , only one of them needs to choose a random backoff to prevent a new collision of the two stations in their next transmission attempt.
\DIFaddbegin \DIFadd{In fact, switching the deterministic station to random behaviour only increases the chance of collision with other deterministic stations.
}\DIFaddend Consequently, the protocol might be improved if deterministic stations kept using a deterministic \DIFaddbegin \DIFadd{even }\DIFaddend backoff after collisions.
The property of using a deterministic backoff after suffering collisions is called \DIFdelbegin \DIFdel{in }\DIFdelend ``stickyness'' \cite{fang2011dlm}.
...
Secondly, once a collision-free schedule is built, it is not possible \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend a channel error \DIFdelbegin \DIFdel{moves }\DIFdelend \DIFaddbegin \DIFadd{to move }\DIFaddend the system back to the random behaviour.
...
Two stations using a deterministic backoff may collide if \DIFdelbegin \DIFdel{one of them miscounts the number of slots}\DIFdelend \DIFaddbegin \DIFadd{their slot boundaries are not aligned}\DIFaddend .
This is a very undesirable situation if both stations ``stick'' to the use of \DIFdelbegin \DIFdel{a }\DIFdelend deterministic backoff after \DIFdelbegin \DIFdel{suffering a collision.
Sticking to the same slot after an unsuccessful transmission will likely result in a new collision }\DIFdelend \DIFaddbegin \DIFadd{colliding, as they will likely collide again }\DIFaddend in the next transmission attempt.

To \DIFdelbegin \DIFdel{attain }\DIFdelend \DIFaddbegin \DIFadd{benefit from }\DIFaddend the advantages of stickiness while preventing the \DIFaddbegin \DIFadd{aforementioned }\DIFaddend potential pitfall, \cite{fang2011dlm} \DIFdelbegin \DIFdel{proposed }\DIFdelend \DIFaddbegin \DIFadd{proposes }\DIFaddend probabilistic stickiness and \cite{barcelo2011tcf} \DIFdelbegin \DIFdel{proposed }\DIFdelend \DIFaddbegin \DIFadd{proposes }\DIFaddend finite stickiness, in which deterministic stations switch back to random behaviour after a given number of \DIFaddbegin \DIFadd{consecutive }\DIFaddend collisions.
...
The paper also presents a comprehensive simulation study which includes realistic ingredients\DIFaddbegin \DIFadd{, }\DIFaddend such as traffic differentiation, \DIFdelbegin \DIFdel{carrier sense }\DIFdelend \DIFaddbegin \DIFadd{carrier-sense }\DIFaddend errors, and channel errors.
...
Many performance aspects are covered in \cite{fang2011dlm}, which offers a comparison among different \DIFdelbegin \DIFdel{learning MAC protocols }\DIFdelend \DIFaddbegin \DIFadd{protocols that converge to collision-free operation}\DIFaddend , and studies the speed of convergence and performance in unsaturated scenarios and in the presence of errors and legacy stations.

Fairness of \DIFdelbegin \DIFdel{the new protocol }\DIFdelend \DIFaddbegin \DIFadd{CSMA/ECA }\DIFaddend with regard to legacy stations is addressed in \cite{barcelo2010fcc}.
...
A detailed performance evaluation of \DIFdelbegin \DIFdel{the new protocol }\DIFdelend \DIFaddbegin \DIFadd{CSMA/ECA }\DIFaddend is offered in \cite{martorell2012pec}.
This paper presents an analytical model and simulations that use realistic channel realizations and the \DIFdelbegin \DIFdel{automated rate fallback }\DIFdelend \DIFaddbegin \DIFadd{Automated Rate Fallback }\DIFaddend (ARF)\DIFdelbegin \DIFdel{mechanism for the selection of the modulation and channel coding scheme}\DIFdelend .
The paper \DIFaddbegin \DIFadd{also }\DIFaddend offers results for \DIFdelbegin \DIFdel{the two handshaking mechanisms in IEEE 802.11: Basic Access (BA) and request-to-send/clear-to-send (RTS/CTS).
...
%DIFDELCMD < %%%
\DIFdel{An exponential packet length distribution is considered and the curves that compare }\DIFdelend CSMA/CA \DIFdelbegin \DIFdel{and CSMA}\DIFdelend \DIFaddbegin \DIFadd{with and without the Request-To-Send}\DIFaddend /\DIFdelbegin \DIFdel{E2CA (See Fig.~\ref{fig:performance}) show a substantial throughput improvement, for both the BA and }\DIFdelend \DIFaddbegin \DIFadd{Clear-To-Send (}\DIFaddend RTS/CTS\DIFdelbegin \DIFdel{the scheme.
The details of the study are available in }%DIFDELCMD < \cite{martorell2012pec}%%%
\DIFdelend \DIFaddbegin \DIFadd{) four-way handshake}\DIFaddend .

\DIFdelbegin \DIFdel{The purpose of ARF is }\DIFdelend \DIFaddbegin \DIFadd{ARF is introduced }\DIFaddend to adapt the transmission rate to the channel conditions \DIFdelbegin \DIFdel{.
...
%DIFDELCMD < %%%
\DIFdel{The problem is that, when BA is used, ARF cannot differentiate between channel errors and collisions.
In any case, it slows down }\DIFdelend \DIFaddbegin \DIFadd{and it reduces }\DIFaddend the transmission rate \DIFdelbegin \DIFdel{.
When ARF is combined }\DIFdelend \DIFaddbegin \DIFadd{after unsuccessful transmissions.
This approach does not work well }\DIFaddend with CSMA/CA \DIFdelbegin \DIFdel{in scenarios with a large number of contenders, the high number of collisions slows down }\DIFdelend \DIFaddbegin \DIFadd{when collision occurs, as ARF misinterprets the failures as channel errors and reduces }\DIFaddend the transmission rate\DIFdelbegin \DIFdel{and therefore the performance is doubly penalized: firstly because of the collisions and secondly because of the slower rates.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This problem is apparent in the BA curve in Fig.~\ref{fig:performance}.
The use of }\DIFdelend \DIFaddbegin \DIFadd{.
This problem can be alleviated by using }\DIFaddend RTS/CTS \DIFdelbegin \DIFdel{involves additional handshaking control frames and makes it possible }\DIFdelend \DIFaddbegin \DIFadd{that can be used }\DIFaddend to differentiate between \DIFdelbegin \DIFdel{channel errors and collisions.
Therefore CSMA/CA-RTS}\DIFdelend \DIFaddbegin \DIFadd{collisions and channel errors, but RTS}\DIFaddend /CTS \DIFdelbegin \DIFdel{outperforms }\DIFdelend \DIFaddbegin \DIFadd{penalizes the performance due to the overheads.
}\DIFaddend CSMA/\DIFdelbegin \DIFdel{CA-BA when the number of contenders is large.
The best performance is obtained by }\DIFdelend \DIFaddbegin \DIFadd{ECA solves the problem by preventing collision, without adding any additional overhead.
The curves in Fig.~\ref{fig:performance} (reproduced from }\cite{martorell2012pec}\DIFadd{) shows that }\DIFaddend CSMA/\DIFdelbegin \DIFdel{E2CA-BA as it prevents collisions and does not suffer the overhead of the extra }\DIFdelend \DIFaddbegin \DIFadd{ECA outperforms CSMA/CA and also CSMA/CA with }\DIFaddend RTS/CTS\DIFdelbegin \DIFdel{packets}\DIFdelend .

%DIF > The paper offers results for the two handshaking mechanisms in IEEE 802.11: Basic Access (BA) and request-to-send/clear-to-send (RTS/CTS).
\DIFaddbegin 

%DIF > An exponential packet length distribution is considered and the curves that compare CSMA/CA and CSMA/E2CA (See the curves in Fig.~\ref{fig:performance} which are reproduced from \cite{martorell2012pec}) show a substantial throughput improvement.
%DIF > The details of the study are available in \cite{martorell2012pec}.

%DIF > The purpose of ARF is to adapt the transmission rate to the channel conditions.
%DIF > In unfavorable channel conditions, ARF uses slower (and more robust) transmission rates.
%DIF > The ARF mechanism simply switches to slower transmission rates after unsuccessful transmissions, as it believes that they are an indication of unfavorable channel conditions.

%DIF > The problem is that, unless RTS/CTS is used, ARF cannot differentiate between channel errors and collisions.
%DIF > In any case, it slows down the transmission rate.
%DIF > When ARF is combined with CSMA/CA in scenarios with a large number of contenders, the high number of collisions slows down the transmission rate and therefore the performance is doubly penalized: firstly because of the collisions and secondly because of the slower rates.

%DIF > This problem is apparent in the CSMA/CA curve in Fig.~\ref{fig:performance} and can be partially alleviated by using RTS/CTS.
%DIF > However, RTS/CTS incurrs in additional overheads.
%DIF > The best performance is obtained by CSMA/E2CA-BA as it prevents collisions and does not suffer the overhead of the extra RTS/CTS packets.


\DIFaddend \section{Conclusion} \label{sec:conclusion}
\DIFaddbegin 

\DIFaddend In this paper we have \DIFdelbegin \DIFdel{presented a tutorial description of CSMA/ECA, a wireless MAC protocol in which the participating devices construct a collision-free schedule.
This protocol requires neither additional processing costs nor message interchange.
All it needs is for the stations to use }\DIFdelend \DIFaddbegin \DIFadd{discussed the use of }\DIFaddend a deterministic backoff after \DIFdelbegin \DIFdel{successful transmissions.
}\DIFdelend \DIFaddbegin \DIFadd{collisions to prevent collisions in WLANs and deliver substantial performance benefits.
The use of a deterministic backoff represents a simple modification to the currently used protocol, thus offering backward compatibility and fair coexistence with already deployed hardware.
}


\DIFadd{Similar techniques have been used to address other problems in wireless networking such as channel assignment, spreading code assignment and channel sensing order assignment in cognitive radio.
}

%DIF > In this paper we have presented a tutorial description of CSMA/ECA, a wireless MAC protocol in which the participating devices construct a collision-free schedule.
%DIF > This protocol requires neither additional processing costs nor message interchange.
%DIF > All it needs is for the stations to use a deterministic backoff after successful transmissions.
\DIFaddend 
...
\DIFdelbegin \DIFdel{In the second part of the paper we have reviewed different contributions that either assess the performance of the system in non-ideal conditions or suggest improvements to deal with the non-idealities.
...
}\DIFdelend %DIF > In the second part of the paper we have reviewed different contributions that either assess the performance of the system in non-ideal conditions or suggest improvements to deal with the non-idealities.
%DIF > We have also discussed the distributed adjustment of the schedule length to accommodate a potentially large number of contenders in a collision-free fashion.
%DIF > And we have introduced stickiness, which reduces the time required to construct the collision-free schedule and increases the robustness of the schedule in scenarios with channel errors or new stations joining the channel.
