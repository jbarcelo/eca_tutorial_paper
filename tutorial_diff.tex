...
\author{
    ...
    Gabriel~Martorell,~%\IEEEmembership{Member,~IEEE,}
    and~David~Malone%~\IEEEmembership{Life~Fellow,~IEEE}% <-this % stops a space
\thanks{J. Barcelo, Azadeh Faridi and Boris Bellalta are with Universitat Pompeu Fabra.\newline
G. Martorell is with Universitat de les Illes Balears.\newline
D. Malone is with the Hamilton Institute, National University of Ireland.\newline
The corresponding author is J. Barcelo. E-mail address: jaume.barcelo@upf.edu Postal Address: Roc Boronat, 138, 08018, Barcelona.}%
}
...
\begin{abstract}
%\boldmath
In wireless local area networks (WLANs), a media access protocol arbitrates \DIFdelbegin \DIFdel{the }\DIFdelend access to the channel.
...
\DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{Furthermore, }\DIFaddend it is also extremely robust as it can quickly recover from network problems such as a short interference burst.
...
The goal is to \DIFdelbegin \DIFdel{distributively construct}\DIFdelend \DIFaddbegin \DIFadd{construct, in a distributed way, }\DIFaddend a collision-free schedule that repeats periodically.
...
We will also briefly mention the generality of the underlying principle and the possibility of using it for diverse problems in the field of radio resource management.
...
Empty slots are relatively short and of constant duration, which is specified by the standard\DIFdelbegin \DIFdel{.
Contrastingly, }\DIFdelend \DIFaddbegin \DIFadd{, and  }\DIFaddend busy slots are of variable length. 
\DIFaddbegin \DIFadd{As an example, the empty slot duration for IEEE 802.11b is 20 $\mu$s and a busy slot can be 1200 $\mu$s long.
}

\DIFaddend Since the stations can use carrier sensing to detect the end of a transmission, it is possible to synchronize the nodes to the end of variable length transmissions.
...
\DIFdelbegin \DIFdel{Contrastingly}\DIFdelend \DIFaddbegin \DIFadd{In contrast}\DIFaddend , wireless devices do not have the possibility to detect a collision while they are transmitting.
