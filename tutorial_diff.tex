...
\DIFaddbegin \DIFadd{In doing so, it decreases the chances of collisions and, under the assumption of fixed packet size, it doubles the throughput of the original Aloha.
}\DIFaddend Reservation Aloha \cite{crowther1973sbc} extends Slotted Aloha with a reservation mechanism.
...
\DIFdelbegin \DIFdel{In those networks }\DIFdelend \DIFaddbegin \DIFadd{The original Aloha network was designed for inter-island communication.
...
When }\DIFaddend where the propagation times are short compared to the duration of the transmission of a packet and all the stations can hear each other's transmissions, empty slots can be \DIFaddbegin \DIFadd{made }\DIFaddend much shorter than busy slots.
...
\DIFaddbegin \DIFadd{We will note that the underlying principles behind our protocol have applicability to other areas of wireless networking.
}\DIFaddend 

\DIFdelbegin \DIFdel{Some }\DIFdelend \DIFaddbegin \DIFadd{As pointed by some }\DIFaddend of the provided references\DIFdelbegin \DIFdel{point out that }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the general framework that \DIFdelbegin \DIFdel{that }\DIFdelend we use in our description of CSMA/ECA has applicability to other problems in the networking and communications field, such as network coding and channel assignment.
...
This technique is called }\DIFaddend CSMA with collision detection (CSMA/CD) \DIFdelbegin \DIFdel{to keep the }\DIFdelend \DIFaddbegin \DIFadd{and keep the duration of }\DIFaddend collision slots very short.
\DIFaddbegin 
...
Nevertheless, we can ignore this exception in our initial discussion and , for the sake of simplicity, assume that the stations always backoff for a random number of slots before transmitting}\DIFdelend \DIFaddbegin \DIFadd{, channel is divided into slots and transmissions are synchronized to slot borders and preceded by a random backoff}\DIFaddend .
...
The construction of the collision-free schedule results in \DIFdelbegin \DIFdel{important }\DIFdelend \DIFaddbegin \DIFadd{significant }\DIFaddend performance gains in terms of throughput\DIFaddbegin \DIFadd{, }\DIFaddend as we will see in the next section\DIFaddbegin \DIFadd{, because the deterministic stations may only collide with random stations and not with one another}\DIFaddend .
...
\DIFadd{Even though there are clear similarities between CSMA/ECA and Reservation Aloha, there are also two marked differences.
The first one is that in Reservation Aloha the slot size is fixed, while in CSMA/ECA the slot size is variable.
The second difference is that in Reservation Aloha there is a reservation while in }\DIFaddend CSMA/ECA \DIFdelbegin \DIFdel{is the close similarity to the protocol that is used in currently deployed devices, }\DIFdelend \DIFaddbegin \DIFadd{there is not.
...
It is remarkable that the performance advantage }\DIFaddend of CSMA/ECA \DIFdelbegin \DIFdel{in next-generation devices}\DIFdelend \DIFaddbegin \DIFadd{does not come at a price of additional signaling or extra overheads}\DIFaddend .
...
Even though CSMA/ECA was initially suggested to prevent collisions in WLANs, \DIFdelbegin \DIFdel{it has been recently shown that }\DIFdelend the underlying mathematical framework is applicable to disparate resource allocation problems in the field of networking, such as channel selection for channel spatial reuse and network coding \cite{duffy2011dcs}.
...
In other words, if there were more stations than columns in Fig. \ref{fig:csma_eca_compact}, it would not be possible to assign the stations to columns in such a way that there \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend a single station in each column.
...
Consider }\DIFdelend \DIFaddbegin \DIFadd{As an example, consider the 3-node network shown in }\DIFaddend Fig. \ref{fig:csma_eca_different_backoff}\DIFdelbegin \DIFdel{in which }\DIFdelend \DIFaddbegin \DIFadd{, where all three }\DIFaddend three stations have reached a collision-free schedule.
...
\DIFaddend If CSMA/ECA is used as described in Sect.~\ref{sec:eca}, the entry of a new contender or a single channel error can disrupt collision-free operation.
...
}\DIFdelend Compare this behaviour to the behaviour observed in Fig. \ref{fig:csma_eca_compact} where the station that successfully transmits in slot 11, suffers a collision in slot 27 and then causes another collision after using a random backoff and transmitting in slot 45.
...
As an example, it can be observed that Fig. \ref{fig:csma_e2ca} converge faster than \ref{fig:csma_eca}.
...
\DIFadd{Under the assumption of ideal clocks, increasing the stickiness increases the robustness.
In fact, it could be possible to use a deterministic backoff forever after a single successful transmission.
...
\DIFdelbegin \DIFdel{Another interesting remark of that paper is the fact }\DIFdelend \DIFaddbegin \DIFadd{Interestingly, the authors remark }\DIFaddend that the implementation of the protocol in the well-known simulator NS-2 required the change of only three lines of code.
