...
Finally, simulation results confirm the advantages of the \DIFdelbegin \DIFdel{proposed approach}\DIFdelend \DIFaddbegin \DIFadd{use of a deterministic backoff after successful transmsissions}\DIFaddend .
\end{abstract}
...
In doing so, it decreases the chances of collisions and, under the assumption of fixed packet size \DIFaddbegin \DIFadd{equal to the above mentioned fixed slot length}\DIFaddend , it doubles the throughput of the original Aloha.
\DIFaddbegin 
...
Then we will review different contributions in this research field to discuss  which are the research challenges, the performance gains\DIFaddbegin \DIFadd{, }\DIFaddend and the scenarios of interest and applicability.
...
The general rule is that collision free operation is reached after all the contending stations successfully transmit \DIFdelbegin \DIFdel{in the same cycle }\DIFdelend \DIFaddbegin \DIFadd{within a single cycle duration}\DIFaddend .
...
In other words, if there were more stations than columns in Fig.~\ref{fig:csma_eca_compact}, it would not be possible to assign the stations to columns in such a way that there \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{would be }\DIFaddend a single station in each column.
...
Secondly, once }\DIFaddend a collision-free \DIFdelbegin \DIFdel{schedule.
And third, it makes the schedule robust to channel errors.
}\DIFdelend \DIFaddbegin \DIFadd{scheduled is built, it is not possible that a channel error moves the system back to the random behaviour.
...
The choice of two deterministic backoffs after a successful transmission is a compromise value that provides robustness against collisions and channel errors and agile recovery in the case of slot drift}\DIFdelend \DIFaddbegin \DIFadd{.
Two stations using a deterministic backoff may collide if one of them miscounts the number of slots.
This is a very undesirable situations if both stations ``stick'' to the use of a deterministic backoff after suffering a collision.
}
...
\DIFadd{Fig.~\ref{fig:csma_e2ca} illustrates the operation of the protocol when deterministic stations ``stick'' to a deterministic backoff after suffering a collision.
It can be observed that in this example, the convergence to collision free operation is faster than in Fig.~\ref{fig:csma_eca_compact}}\DIFaddend .

%DIF > If CSMA/ECA is used as described in Sect.~\ref{sec:eca}, a single channel error or the entry of new contender can disrupt collision-free operation.
%DIF > To address this problem, stickiness was introduced in \cite{fang2011dlm} and later discussed in \cite{barcelo2011tcf} in a protocol called CSMA/E2CA.
%DIF > In CSMA/E2CA, a station that is using a deterministic backoff will only switch to a random behaviour if it suffers two unsuccessful transmissions in two consecutive transmission attempts.
%DIF > 
%DIF > The operation of CSMA/E2CA is illustrated in Fig.~\ref{fig:csma_e2ca}.
%DIF > In this figure, there is a station that successfully transmits in slot~11.
%DIF > This station uses a deterministic backoff and then it suffers an unsuccessful  transmission in its next transmission attempt in slot 27.
%DIF > Despite the collision, this station sticks to using a deterministic backoff because the CSMA/E2CA protocol states that a deterministic backoff is used for two consecutive times after a successful transmission.
%DIF > Compare this behaviour to the behaviour observed in Fig.~\ref{fig:csma_eca_compact} where the station that successfully transmits in slot 11, suffers a collision in slot 27 and then causes another collision after using a random backoff and transmitting in slot 45.
%DIF > 
%DIF > The extra stickiness \cite{barcelo2011tcf} offers three advantages.
%DIF > First, it accelerates the convergence to collision free operation.
%DIF > As an example, it can be observed that CSMA/E2CA in Fig. \ref{fig:csma_e2ca} converges faster than CSMA/ECA in \ref{fig:csma_eca_compact}.
%DIF > Second, it prevents cascading or chain-reaction effects in which the entry of single user completely breaks a collision-free schedule.
%DIF > And third, it makes the schedule robust to channel errors.
%DIF > 
%DIF > Under the assumption of ideal clocks, increasing the stickiness also increases the robustness.
%DIF > In this ideal situation, it would make sense to use a deterministic backoff forever after a single successful transmission.
%DIF > The problem is that real clocks suffer clock drifts \cite{gong2012asd} and, as a result, two stations that have successfully transmitted in consecutive slots may end up colliding in the next transmission slot.
%DIF > This event is not likely, but it is possible.
%DIF > If it happens, then the stickiness is an obstacle to recover normal operation.
%DIF > The choice of two deterministic backoffs after a successful transmission is a compromise value that provides robustness against collisions and channel errors and agile recovery in the case of slot drift.
\DIFaddbegin 
...
One of the characteristics of CSMA/ECA is that reaching a collision-free schedule may require several \DIFdelbegin \DIFdel{transmission rounds}\DIFdelend \DIFaddbegin \DIFadd{schedule cycles}\DIFaddend .
...
This paper presents an analytical model and simulations that use realistic channel realizations and the automated rate fallback \DIFaddbegin \DIFadd{(ARF) }\DIFaddend mechanism for the selection of the modulation coding scheme.
...
BA is simpler as it includes only a transmission and an acknowledgement.
In RTS/CTS, the transmission and the acknowledgement are preceded by the RTS and CTS signaling frames.
}
...
By doing so, it is guaranteed that \DIFdelbegin \DIFdel{two }\DIFdelend stations that have successfully transmitted in their last transmission attempt will not collide \DIFdelbegin \DIFdel{with each other }\DIFdelend \DIFaddbegin \DIFadd{among them in }\DIFaddend their next transmission attempt.
