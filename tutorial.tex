...
If a deterministic station backoff collides with a random station, only one of them needs to choose a random backoff to prevent a new collision of the two stations in their next transmission attempt.
In fact, switching the deterministic station to random behaviour only increases the chance of collision with other deterministic stations.
Consequently, the protocol might be improved if deterministic stations kept using a deterministic even backoff after collisions.
...
Secondly, once a collision-free schedule is built, it is not possible for a channel error to move the system back to the random behaviour.
...
This paper presents an analytical model and simulations that use realistic channel realizations and the Automated Rate Fallback (ARF).
The paper also offers results for CSMA/CA with and without the Request-To-Send/Clear-To-Send (RTS/CTS) four-way handshake.

ARF is introduced to adapt the transmission rate to the channel conditions and it reduces the transmission rate after unsuccessful transmissions.
This approach does not work well with CSMA/CA when collision occurs, as ARF misinterprets the failures as channel errors and reduces the transmission rate.
This problem can be alleviated by using RTS/CTS that can be used to differentiate between collisions and channel errors, but RTS/CTS penalizes the performance due to the overheads.
CSMA/ECA solves the problem by preventing collision, without adding any additional overhead.
The curves in Fig.~\ref{fig:performance} (reproduced from \cite{martorell2012pec}) shows that CSMA/ECA outperforms CSMA/CA and also CSMA/CA with RTS/CTS.
...
%An exponential packet length distribution is considered and the curves that compare CSMA/CA and CSMA/E2CA (See the curves in Fig.~\ref{fig:performance} which are reproduced from \cite{martorell2012pec}) show a substantial throughput improvement.
...
%The problem is that, unless RTS/CTS is used, ARF cannot differentiate between channel errors and collisions.
...
%This problem is apparent in the CSMA/CA curve in Fig.~\ref{fig:performance} and can be partially alleviated by using RTS/CTS.
%However, RTS/CTS incurrs in additional overheads.
...
In this paper we have discussed the use of a deterministic backoff after collisions to prevent collisions in WLANs and deliver substantial performance benefits.
