...
In CSMA/ECA, the number of collisions is initially reduced before the different stations construct a collision-free schedule.
...
Finally, simulation results confirm the advantages of the proposed approach.
\end{abstract}
...
Recently, different research initiatives have suggested to combine the advantages of Reservation Aloha networks and CSMA networks (e.g., \cite{barcelo2008lba,he2009srb,barcelo2010fcc,fang2011dlm,barcelo2011tcf,martorell2012pec}). 
...
Subfigure Fig.~\ref{fig:csma_ca} represents two stations competing for the channel using CSMA/CA.
...
The general rule is that collision free operation is reached after all the contending stations successfully transmit in the same cycle.
...
If CSMA/ECA is used as described in Sect.~\ref{sec:eca}, a single channel error or the entry of new contender can disrupt collision-free operation.
...
The extra stickiness \cite{barcelo2011tcf} offers three advantages.
...
Under the assumption of ideal clocks, increasing the stickiness also increases the robustness.
...
The problem is that real clocks suffer clock drifts \cite{gong2012asd} and, as a result, two stations that have successfully transmitted in consecutive slots may end up colliding in the next transmission slot.
