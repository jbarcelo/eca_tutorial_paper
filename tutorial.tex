...
In current IEEE 802.11 WLANs (popularly known by its certification name, WiFi), carrier sense multiple access with collision avoidance (CSMA/CA) is used.
The contending stations that use CSMA/CA do not make any attempt to agree on a collision-free schedule.
A collision-free schedule is advantageous because the stations take turns in transmitting and collisions do not occur.
Carrier sense multiple access with enhanced collision avoidance (CSMA/ECA) is a subtle variant of the well-known CSMA/CA in which the different stations construct a collision-free schedule.
...
The construction of the collision-free schedule automatically translates into performance gains.
The first part of the article is of tutorial nature, offering an introduction to the basic operation of CSMA/ECA and describing the benefits of this approach in a qualitative manner.
The second part of the article surveys the main contributions to this research field, briefly summarizing the main challenges and potential solutions, and also introducing variants and derivatives of CSMA/ECA.
\end{abstract}
...
\IEEEPARstart{T}{he} sharing of a communication medium as a common resource by multiple stations is a classic communications problem. 
...
In these article, we will cover random (and semi-random) multiple access protocols that are faithful to the simplicity, low overhead and distributed nature of the early Aloha protocol.
...
A media access protocol that is closely related to Reservation Aloha is Packet Reservation Multiple Access \cite{goodman1989prm} where periodic traffic uses reservation and the remaining traffic uses random access.
A common characteristic of Slotted Aloha, Reservation Aloha, and Packet Reservation Multiple Access is that they all use fixed size slots.
...
\section{The Construction of a Collision-Free Schedule in CSMA/ECA}
...
A quick clarification is in order to contrast CSMA/CA and CSMA/CD (CSMA with Collision Detection).
The former is used in wireless networks and is characterized by long collision slots.
The latter, which is used in wired networks, has the capability of immediately detecting collisions (as its name clearly indicate) and therefore reduces the length of collision slots to a minimum.
The focus of this article is on wireless networks where collision detection is not practical and therefore the subject of our discussion is CSMA/CA.
...
Imagine that two stations $STA 1$ and $STA 2$ successfully transmit in slot $A$ and in slot $B$, respectively,  and then they both backoff for the same number of slots $V$.
...
A random backoff value is used after collisions and a deterministic backoff value is used after successful transmissions.
...
Compare the behaviour of CSMA/CA in Fig. \ref{fig:csma_ca} to the behaviour of CSMA/ECA in Fig. \ref{fig:csma_eca}.
...
Another advantage of CSMA/ECA is the close similarity to the protocol that is used in currently deployed devices.
...
\section{Refinements, performance and mathematical framework}
\label{sec:survey}
...
Some of the issues we will be looking at are backward compatibility, operation in the presence of errors and operation in networks with a very large number of contenders. 
...
When the collision-free schedule is eventually built, the system moves to a deterministic steady state.
...
Some improvements on top of the basic idea are presented in \cite{fang2011dlm}. 
...
Additionally, the station that doubles its deterministic backoff will also double the number of packets that are transmitted every time that it accesses the medium.
...
Consider Fig. \ref{fig:csma_eca_different_backoff} in which three stations have built a collision-free schedule.
Notice that the schedule length of the bottom station is twice as long as the schedule length of the other two stations.
The reason might be that this station has suffered a large number of collisions or channel errors and has decided that the number of contenders might be high and taken action by doubling its schedule length.
...
As a summary, we have provided an overview of a promising collaborative approach that could substantially reduce the problem of collisions in next generation WLANs.
